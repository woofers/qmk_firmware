...
static uint16_t function_is_down = 0;
static uint16_t cancel_esc = 0;
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
  switch (keycode) {
    case FUNC: {
      ...
      if (record->event.pressed) {
        ...
        if (timer_elapsed(key_timer) < LAYER_TIMEOUT && !cancel_esc) {
          tap_code(KC_ESC);
        }
        function_is_down = 0;
        cancel_esc = 0;
  ...
  if (record->event.pressed && function_is_down && keycode != FUNC) {
    cancel_esc = 1;
  }
  return true;
};
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  ...
  [_LOWER] = LAYOUT_60_tsangan_hhkb(
    ...
    _______, _______, KC_FORWARD, _______,   _______, _______, KC_YANK, KC_UNDO, KC_HOME,  KC_END,  KC_PASTE, _______, _______,   RESET,
    _______, _______, _______,    KC_CUT,    _______, KC_TOP,  KC_LEFT, KC_DOWN, KC_UP,    KC_RGHT, _______,  _______, _______,
  ...
  [_FN1] = LAYOUT_60_tsangan_hhkb(
    ...
    RGB_TOG, RGB_MOD, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______,  RESET,
